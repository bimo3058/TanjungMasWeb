...
const checklist = computed(() => [
  { label: 'Nama usaha diisi', done: !!values.value.nama_usaha.trim() },
  { label: 'Nama pemilik diisi', done: !!values.value.nama_pemilik.trim() },
  ...
  { label: 'Nomor telepon diisi', done: !!values.value.nomor_telepon.trim() },
...
const checklistMeta = computed(() => [
  ...
  // Slug dihitung ulang dari nama usaha saat menyimpan, jadi ini tautan yang
  // akan terbentuk — bukan slug lama yang tersimpan.
  { label: 'Tautan publik', value: `/umkm/${slugify(values.value.nama_usaha) || 'nama-usaha'}` },
])
